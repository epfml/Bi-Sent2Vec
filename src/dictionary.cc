...
bool Dictionary::discard(int32_t id, real rand) const {
  assert(id >= 0);
  assert(id < nwords_);
  if (args_->model == model_name::bisent2vec) return false;
...
int32_t Dictionary::getLine(std::istream& in,
  ...
  while (readWord(in, token)) {
    if (token == EOS && (args_-> model == model_name::bisent2vec)){
       break;
    }
    ...
    if (token == EOS) break;
    if (ntokens > MAX_LINE_SIZE && args_->model != model_name::bisent2vec) break;
  }
  return ntokens;
}
