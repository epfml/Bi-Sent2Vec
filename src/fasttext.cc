...
void FastText::saveVectors() {
  ...
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    ...
    getVector_ckpt(vec, i);
    ofs << word << " " << vec << std::endl;
  }
  ofs.close();
}

void FastText::saveVectors(int32_t checkpoint) {
  std::ofstream ofs(args_->output + "_Chk" + std::to_string(checkpoint) + ".ckpt.vec");
  if (!ofs.is_open()) {
    std::cerr << "Error opening file for saving vectors." << std::endl;
    exit(EXIT_FAILURE);
  }
...
void FastText::transgram(Model& model, real lr,
                        const std::vector<int32_t>& line) {
  ...
  for (int32_t w = 0; w < line1.size(); w++) {
    ...
    word.push_back(line1[w]);
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line1.size()) {
        model.update(word, line1[w + c], lr);
      }
    }
    for (int32_t i=0; i<line2.size(); ++i){
        model.update(word, line2[i], lr);
    }
  }
  for (int32_t w = 0; w < line2.size(); w++) {
    int32_t boundary = uniform(model.rng);
    std::vector<int32_t> word;
    word.push_back(line2[w]);
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line2.size()) {
        model.update(word, line2[w + c], lr);
      }
    }
    for (int32_t i=0; i<line1.size(); ++i){
        model.update(word, line1[i], lr);
    }
  }
}
...
void FastText::bisent2vec(Model& model, real lr, const std::vector<int32_t>& line){
  if (line.size() <= 1) return;
  ...
  for (int32_t i=0; i<line1.size(); ++i){
    if (uniform(model.rng) > dict_->getPDiscard(line1[i]) || dict_->getTokenCount(line1[i]) < args_->minCountLabel)
      continue;
    context = line1;
    context[i] = 0;
    dict_->addNgrams(context, args_->wordNgrams, args_->dropoutK, model.rng);
    model.update(context, line1[i], lr);
    context = line2;
    dict_->addNgrams(context, args_->wordNgrams, args_->dropoutK, model.rng);
    model.update(context, line1[i], lr);
  ...
  for (int32_t i=0; i<line2.size(); ++i){
    if (uniform(model.rng) > dict_->getPDiscard(line2[i]) || dict_->getTokenCount(line2[i]) < args_->minCountLabel)
      continue;
    context = line2;
    context[i] = 0;
    dict_->addNgrams(context, args_->wordNgrams, args_->dropoutK, model.rng);
    model.update(context, line2[i], lr);
    context = line1;
    dict_->addNgrams(context, args_->wordNgrams, args_->dropoutK, model.rng);
    model.update(context, line2[i], lr);
  }
...
void FastText::textVectors() {
  ...
  while (std::cin.peek() != EOF) {
    ...
    vec.zero();
    if (args_->model == model_name::bisent2vec){
...
void FastText::getVector_ckpt(Vector& vec, int32_t wordIdx) const {
  const std::vector<int32_t>& ngrams = dict_->getNgrams(wordIdx);
  vec.zero();
  vec.addRow(*input_, wordIdx);
}
...
void FastText::trainThread(int32_t threadId) {
  ...
  while (tokenCount < args_->epoch * ntokens) {
    ...
    if (args_->model == model_name::bisent2vec) {
      bisent2vec(model, lr, line);
    } else if (args_->model == model_name::cbow) {
      cbow(model, lr, line);
    } else if (args_->model == model_name::transgram) {
      transgram(model, lr, line);
    }
...
void FastText::loadVectors(std::string filename) {
  ...
  for (size_t i = 0; i < n; i++) {
    std::string word;
    in >> word;
    if (dict_->getId(word) != -1){
      words.push_back(word);
      dict_->add(word);

      for (size_t j = 0; j < dim; j++) {
        in >> mat->data_[(words.size() -1) * dim + j];
      }
...
void FastText::loadOutputVectors(std::string filename) {
  ...
  for (size_t i = 0; i < words.size(); i++) {
    ...
    if (idx < 0 || idx >= dict_->nwords()) continue;
    for (size_t j = 0; j < dim; j++) {
      output_->data_[idx * dim + j] = mat->data_[i * dim + j];
    }
  }
...
void FastText::train(std::shared_ptr<Args> args) {
  ...
  if (args_->pretrainedVectors.size() != 0) {
    loadVectors(args_->pretrainedVectors);
    loadOutputVectors(args_->pretrainedVectors);
  } else {
